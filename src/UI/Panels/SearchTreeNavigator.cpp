...
namespace Strawberry::Accoutrement
{
	...
	void SongDatabaseListContentsManager::AddSong(size_t songIndex)
	{
		...
		for (int i = 1; i < mSearchBuffer.size(); i++)
		{
			...
			const bool matchesArtist = song.GetArtist().HasValue() && song.GetArtist().Value().find(mSearchBuffer[i].first) != StringType::npos;
	...
	int32_t SongDatabaseListContentsManager::GetStringDifference(SongDatabaseListContentsManager::StringType& a, SongDatabaseListContentsManager::StringType& b)
	{
		StringType* larger;
		StringType* smaller;
		if (a.Len() > b.Len())
	...
	int32_t SongDatabaseListContentsManager::GetLongestPrefixLength(SongDatabaseListContentsManager::StringType& a, SongDatabaseListContentsManager::StringType& b)
	{
