...
namespace Strawberry::Accoutrement
{
    ...
    Core::Optional<std::pair<Discord::Snowflake, Discord::Snowflake> > ChannelSelector::GetSelectedChannel() const
    {
        ...
        auto channelId = static_cast<SnowflakeClientData*>(channelChoice->GetClientObject(channelSelectionIndex))->
                Get();
    ...
    void ChannelSelector::OnConnect(wxCommandEvent& event)
    {
        ...
        if (Bot::Get() && !alreadyConnected)
        {
            ...
            mConnectButton->Disable();
            QueueEvent(new ConnectedToVoice(*Bot::Get()->GetBot()->GetGuild(guildId),
                                            *Bot::Get()->GetBot()->GetChannel(channelId)));
        }
    }
    ...
    void ChannelSelector::UpdateConnectButton()
    {
        ...
        {
            auto [guildId, channelId] = selection.Unwrap();
            auto connectedGuild       = Bot::Get()->GetBot()->GetVoiceConnection().Map([](const auto& x)
            {
                return x->GetGuild();
            });
            auto connectedChannel = Bot::Get()->GetBot()->GetVoiceConnection().Map([](const auto& x)
            {
                return x->GetChannel();
            });
